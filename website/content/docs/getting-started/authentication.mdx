...
For production applications with user-specific access, use OAuth authentication.

### Setup OAuthTokenHandler
...
def save_tokens(tokens):
    """Save refreshed tokens to database/storage."""
    # Save to database, config file, etc.
    database.save_tokens(tokens)

handler = OAuthTokenHandler(
    access_token="ntn_...",  # From OAuth flow
    refresh_token="...",     # From OAuth flow
    client_id="your-client-id",
    client_secret="your-client-secret",
    on_refresh=save_tokens  # Callback for token refresh
)
...
1. **User Authorization**: Redirect user to Notion's OAuth URL
2. **Authorization Code**: Notion redirects back with `code` parameter
3. **Exchange Code**: Exchange code for access and refresh tokens
4. **Use SDK**: Initialize NotionClient with OAuthTokenHandler

```python
import httpx
import base64

async def exchange_code_for_tokens(code, client_id, client_secret):
...
<Callout type="warning">
  **Critical**: Always implement `on_refresh` callback to persist tokens. Without it, tokens won't be saved after refresh.
</Callout>

See: [OAuth](../sdk/oauth) for complete OAuth reference, [Token Refresh](../advanced/token-refresh) for advanced topics.
