...
- `CommentCollection` - Manage comments
...
api.comments   # CommentCollection
```

### 3. High-Level SDK (NotionClient)

<Box /> **NotionClient** provides a higher-level SDK with intelligent caching and autonomous entities.

**Key Features**:
- **Automatic Caching** - Pages, databases, users cached for performance
- **Autonomous Entities** - Work with entities directly or via managers
- **Convenient Managers** - Shortcuts for common operations
- **Type-Safe Properties** - Rich property access on entities
...
client.databases  # DatabaseManager
client.users      # UserManager
client.comments   # CommentsManager

# Direct cache access
client.page_cache       # Page cache
client.database_cache   # Database cache
client.user_cache       # User cache
client.comment_cache    # Comment cache
```

**SDK Entities** are autonomous - can be used independently:

```python
from better_notion._sdk.models import Page, Database, Block

# Get page directly (uses cache)
page = await Page.get("page_id", client=client)

# Access properties
print(page.title)  # Type-safe!
...
- **SDK (NotionClient)**: Building applications, need caching, want type safety
- **API (NotionAPI)**: Building custom abstractions, need raw responses, maximum control

See: [API vs SDK](../api-vs-sdk) for detailed comparison.
